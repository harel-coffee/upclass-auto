...
def parse_from_url(url, db=None):
    pre_text = {}
    if db == 'pmc':
        pre_text = parse_pmc(url)
    else:
        pre_text = parse_pubmed(url)
...
with open(test_set) as test_file:
    for line in test_file:
        ...
        try:
            feat, articles, accessions = tag_article(models, accession, pmid, articles=articles, accessions=accessions,
                                                     mtype=mtype)
            docs.append(pmid + '_' + accession[0])
            regressors.append(feat[0])
